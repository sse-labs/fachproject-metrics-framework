...
class VrefAnalysisTest extends FlatSpec with Matchers{
  ...
  "The VrefAnalysis" must "calculate valid results for single JARs" in {
    ...
    val resultBoxInfo = AnalysisTestUtils.runSingleFileAnalysis(analysisToTest, fileToTestBox, appConfig, Map(Symbol("infozuvariablen") -> true))
    val metricResultBoxInfo = resultBoxInfo.head
    ...
    val resultBoxNoThis = AnalysisTestUtils.runSingleFileAnalysis(analysisToTest, fileToTestBox, appConfig, Map(Symbol("no-this") -> true))
    val metricResultBoxNoThis = resultBoxNoThis.head
    ...
    val resultH2noClass = AnalysisTestUtils.runSingleFileAnalysis(analysisToTest, fileToTestH2, appConfig, Map(Symbol("no-class") -> true))
    val metricResultH2noClass = resultH2noClass.head
    ...
    val resultGsonNoMethod = AnalysisTestUtils.runSingleFileAnalysis(analysisToTest, fileToTestGSON, appConfig, Map(Symbol("no-methoden") -> true))
    val metricResultGsonNoMethod = resultGsonNoMethod.head
