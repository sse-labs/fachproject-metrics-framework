...
class InternalStabilityAnalysisTest extends FlatSpec with Matchers{
  ...
  "The InternalStabilityAnalysis" must "calculate valid results for multiple JARs" in {
    ...
    list.foreach(elem => if (elem.entityIdent.equals("Difference between: gson-1.4.jar and gson-1.5.jar")) {
      ldouble = elem.metricValue
    })
    val ergebniss = Math.round(ldouble * decimal) / decimal
    assert(ergebniss == 0.94493)
    list.foreach(elem => if (elem.entityIdent.equals("Difference between: gson-2.2.jar and gson-2.4.jar")) {
      ldouble = elem.metricValue
    })
